...
(function () {
  ...
  var labels = {
    ...
    math: ["数学／統計", "Math / Statistics"],
    technology: ["技術／AI", "Technology / AI"],
    society: ["社会分析", "Society"],
    notes: ["お知らせ", "Updates"],
  };
  ...
  function photoCount() {
    ...
    document.querySelector(".photo-count").textContent =
      document.querySelectorAll(".journal-photo-card").length +
      (en ? " records " : "件 ") +
      (photos.open ? "−" : "＋");
  }
  ...
  function render() {
    ...
    filters.forEach(function (link) {
      ...
      if (category === key) link.setAttribute("aria-current", "true");
      else link.removeAttribute("aria-current");
    });
    document.querySelector("#journal-result-heading").textContent =
      key === "all" ? (en ? "All articles" : "すべての記事") : labels[key][en ? 1 : 0];
    document.querySelector("#journal-count").textContent =
      count +
      (en
        ? count === 1
    ...
    document.querySelector(".journal-empty-title").textContent = en
      ? "Articles are on the way"
      : "記事は準備中です";
    document.querySelector(".journal-empty-description").textContent =
      key === "math"
        ? en
          ? "Notes on mathematics and statistics will appear here."
          : "数学や統計の学習ノートを、ここにまとめていきます。"
        : en
          ? "Notes on AI and other technologies will appear here."
          : "AIやさまざまな技術の仕組み・発展を、ここにまとめていきます。";
    document.querySelector(".journal-reset").textContent = en
      ? "View all articles →"
      : "すべての記事を見る →";
    document.querySelector(".journal-about").textContent = en ? "About the author →" : "自己紹介 →";
    document.querySelector(".journal-bio").textContent = en
      ? "Reading research, learning mathematics, and thinking about technology and society. Notes from an ongoing learning journey."
      : "研究を読み、数学を学び、技術と社会を考える。学びの途中のメモを残しています。";
  }
